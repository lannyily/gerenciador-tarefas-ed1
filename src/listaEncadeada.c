    ...
    while (atual != NULL) 
    {
        ...
        while (tarefaAtual != NULL) {
            ...
            if (tarefaAtual->id == idTarefa){
                ...
                // Remove a tarefa da lista de tarefas da dat
                ...
                if (atual->cont == 0) {
                    ...
                    free(atual);            // Libera a memória da data
                }
...
void moverTarefasVencidas(DataTarefa** listaData, DataTarefa** listaPendentesVencidas){
    ...
    while (atual != NULL) {
        if (compararDatas(atual->data, dataHoje) < 0){  // Se a data da tarefa for anterior à data atual.
...
int comparaString(char* nomeBusca, char* nome){
    if(*nomeBusca != 0){                                                    // Se ainda restarem caracteres para comparar.
        if (*nome == 0) return 0;                                           // Se o nome de destino terminou antes.
        if (*nomeBusca == *nome) return comparaString(++nomeBusca, ++nome); // Compara os caracteres e chama recursivamente.
        return 0;                                                           // Se os caracteres forem diferentes, retorna falso.
    } else {
        return (*nome == 0);                                                // Se ambos os nomes terminarem juntos, retorna verdadeiro.
    }
}
...
void buscarTarefaNome(DataTarefa* listaData, char* nomeBusca){
    while(listaData != NULL){
        ...
        while (tarefa != NULL){
            if(comparaString(nomeBusca, tarefa->descricao)){    // Se a descrição da tarefa coincidir com a busca.
...
void dataAtual(char* dataHoje){
    ...
    struct tm tm = *localtime(&tempo);  // Converte para a estrutura tm.
        ...
        if (strcmp(aux->data, dataHoje) == 0) {     // Se a data da entrada for igual à data atual.
            ...
            while (tarefa != NULL) {
                ...
                if (listaOrdenada == NULL) {
                    ...
                    while (temp->prox != NULL) {             // Caso contrário, insere no final da lista ordenada.
                        temp = temp->prox;
                    }
...
void inserirTarefa(TAREFA** lista, int id, char* status, char* descricao, int prioridade){
    ...
    // Se a lista estiver vazia, a tarefa se torna o primeiro elemento.
    if (*lista == NULL){
        *lista = novaTarefa;
    } else {
        TAREFA* temp = *lista;
        // Se já houver tarefas na lista, percorre até o final.
        while (temp->prox != NULL){
            temp = temp->prox;
        }
        temp->prox = novaTarefa;        // Adiciona a nova tarefa no final da lista.
    }
}
...
TAREFA* removerTarefa(TAREFA* lista, Alteracao* pilha, int idBusca) {
    ...
    return lista;   // Se a tarefa não for encontrada, retorna a lista original.
}
...
DataTarefa* removerTarefaData(DataTarefa* listaData, Alteracao* pilha, int idBusca) {
    ...
    while (auxData != NULL) {
        int tamanhoAnterior = listaTamanho(auxData->tarefas);               // Obtém o tamanho anterior da lista de tarefas.
        auxData->tarefas = removerTarefa(auxData->tarefas, pilha, idBusca); // Remove a tarefa da lista de tarefas.
        int tamanhoAtual = listaTamanho(auxData->tarefas);                  // Obtém o tamanho atual da lista de tarefas.

         // Se o tamanho mudou, significa que a tarefa foi removida.
   ...
   printf("Digite o novo nome (ou pressione Enter para manter): ");
    ...
    while (1) {
        if (fgets(novaDescricao, sizeof(novaDescricao), stdin) != NULL) {
            ...
            if (strlen(novaDescricao) > 300) {                  // Se a descrição for maior que 300 caracteres.
                printf("A descricao nao pode ter mais que 300 caracteres! Tente novamente! ");
            } else {
                strcpy(tarefa->descricao, novaDescricao);       // Atualiza a descrição da tarefa.
                break;
            }
        }
    }
...
void buscarTarefa(DataTarefa* listaData, Alteracao* pilha, int idbusca){
    while (listaData != NULL) {
        TAREFA* tarefa = listaData->tarefas;
        while (tarefa != NULL) {
            if (tarefa->id == idbusca){         // Se encontrar a tarefa com o ID buscado.
